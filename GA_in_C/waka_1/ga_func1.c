...
void disp_2d(int arr[][input_cols],int m,int n){
    for(int i=0;i<m;i++){
        for(int j=0;j<n;j++){
            printf("%d ",arr[i][j]);
        }
...
void read_act_output(){
    FILE *csv_file=fopen("output_waka1.csv","r");
    char Buff[1000000],*line,*token;
    int row=0;
    int col=0;
    
    line=fgets(Buff,1000000,csv_file);
    
    token=strtok(line,",");
    col=0;
    while(token != NULL){
        actual_output[row++]=atoi(token);
        token=strtok(NULL,",");
    }
...
void read_input(){
    FILE *csv_file=fopen("input_waka1.csv","r");
    char Buff[1000000],*line,*token;
    int row=0;
    int col=0;
...
double get_error(int key[]){
    ...
    for(int i=0;i<output_rows;i++){
        // printf("Output %d :%d ,\n",actual_output[i],curr_output[i]);
        temp += square(actual_output[i]-curr_output[i]);
        // printf("tmp %lf\n",temp);
...
struct Node *fitness(struct chrom samples[],int n,struct Node *map){
    ...
    for(int i=0;i<n;i++){
        ...
        waka1_ga_func(samples[i].key);  //curr_output will be modified
        err =get_error(samples[i].key);
        // printf("err %d\n",err);
...
void custom_mutation(struct chrom *sample,struct chrom *child){
    ...
    flip_bits(child,32,63);
    flip_bits(child,64,64);
